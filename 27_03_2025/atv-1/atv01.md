...
Resposta: O operador ? permite que uma variável seja tratada como não obrigatória, podendo receber um valor nulo
...
Resposta: Caso não seja implementado um construtor, o erro está em considerar o atributo ano como obrigatório (não nulo). Nesse caso, o atributo ano deve ter o operador ? em sua tipagem
````
class Carro {
  String? marca;
  int? ano;
...
Comentar: Operador de coalescência nula (??)
````
import 'dart:io';
...
Comentar: valores padrão so podem ser definidos diretamente em construtores nomeados
````
class Aluno {
  String nome;
  double nota;
...
Resposta: Métodos get e set em Dart são usados para receber e atribuir valores à atributos de classes, entretanto eles só são relevantes em Dart em contextos de validação de dados ou encapsulamento de atributos privados.
...
## 14. Modifique a classe abaixo para que o set de peso não permita valores negativos:
````
class Pessoa {
  double _peso;
